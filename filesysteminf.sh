...
{
	echo "No se pudo ejecutar esa opción, para obtener ayuda ejecute el script junto con -h."
        echo -n "$1" 1>&2
        exit 1
}
			...
			do
				...
				then
  				...
  				if [[ "$a2_ejercicio" ]]; then								 # si se cumple la condición de que existe un archivo de dispositivo, entonces se hace el nºmayor,menor.
					...
					muestra+="$user ${a2_ejercicio} ${contador} ${cuenteo-1} ${c_ejercicio} ${mayor} ${menor}\n"
					cuenteo=0 # le damos valor 0 para que reitere desde este valor.
				fi
			done
		fi
